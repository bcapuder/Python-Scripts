...
threshold=range(3200,10000,200)
...
time.sleep(2)
r = requests.get('http://search.3taps.com/?auth_token=2506e1aa51e77ceaee767fb5c56135e5&rpp=100&location.country=USA&retvals=id,account_id,source,category,location,external_id,external_url,timestamp,timestamp_deleted,expires,language,price,currency,annotations,status,state,immortal,deleted&category_group=RRRR&tier='+str(tiernum)+'&anchor=1253900449&page='+str(pagecounter))
for thresh in threshold:
    newdict={}
    while r.json()['next_page']>0 and pagecounter<thresh:
        time.sleep(2)
        r = requests.get('http://search.3taps.com/?auth_token=2506e1aa51e77ceaee767fb5c56135e5&rpp=100&location.country=USA&retvals=id,body,heading,account_id,source,category,location,external_id,external_url,timestamp,timestamp_deleted,expires,language,price,currency,annotations,status,state,immortal,deleted&category_group=RRRR&anchor=1253900449&tier='+str(tiernum)+'&page='+str(pagecounter))
        for post in r.json()['postings']:
            ...
            if 'location' in post.keys():
                ...
                if 'state' in post['location'].keys(): 
                    outputdict['state']=str(post['location']['state']).replace('USA-','')
                else:
                    outputdict['state']=''
